...
const stripe = require("stripe")("sk_test_51PvFPH1adOqTPZqxBpHlcFZ8OYxCXpZgbHWyMSyuid6an3sgIywIqhJrOt8cUZ0kBhJe7RZEXcm6DZ0nlxJIQ2wl00FKEEv8GI"); // Use environment variable for the secret key
...
app.post("/api/create-checkout-session", async (req, res) => {
    ...
    try {
        ...
        const lineItems = products.map((product) => {
            ...
            const imageUrl = product.photoUrl.startsWith('http') || product.photoUrl.startsWith('https') 
                ? product.photoUrl 
                : `http://localhost:3000${product.photoUrl}`; // Adjust this based on your server setup

            console.log(imageUrl)
            
            console.log(`Product: ${product.productName}, Price: ${product.productPrice}, Unit Amount (cents): ${unitAmountCents}`);
            return {
                price_data: {
