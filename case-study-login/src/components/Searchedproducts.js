...
import { GET_PRODUCTS_BY_NAME, GET_CART_TABLE, CREATE_OR_UPDATE_CART, UPDATE_CART_ITEM, GET_PRODUCTS_BY_CATEGORY } from '../Apollo/queries'; // Adjust the import path if necessary
import { Toaster, toast } from 'sonner';
import { useNavigate, Link } from 'react-router-dom';
const SearchedProducts = () => {
    const searchString= "novv"
    const [selectedProduct, setSelectedProduct] = useState(null);
    //const [selectedCategory, setSelectedCategory] = useState(null);
    const userId = 1;//parseInt(localStorage.getItem("userData"), 10);
    const navigate = useNavigate();
    const { loading: productsLoading, error: productsError, data: productsData } = useQuery(GET_PRODUCTS_BY_NAME, {
    ...
    return (
        <div>
            ...
            <div className='product-card'>
                {productsData.productsByMatchingString.map((product) => (
